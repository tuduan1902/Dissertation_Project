...
const double wheelbase = 0.3;               //Wheelbase, in m
const double encoder_cpr = 2400;               //Encoder ticks or counts per rotation
const double speed_to_pwm_ratio = 0.0005951;    //Ratio to convert speed (in m/s) to PWM value. It was obtained by plotting the wheel speed in relation to the PWM motor command (the value is the slope of the linear function).
...
// PID Parameters
const double PID_left_param[] = { 1, 0, 0.0025 }; //Respectively Kp, Ki and Kd for left motor PID
const double PID_right_param[] = { 1, 0, 0.0025 }; //Respectively Kp, Ki and Kd for right motor PID
...
// Initialize both motors
  L298NX2 motors(EN_A, IN1_A, IN2_A, EN_B, IN1_B, IN2_B); //right/
...
void setup() {
  ...
  PID_leftMotor.SetSampleTime(1);
  PID_rightMotor.SetSampleTime(1);
...
void loop() {
  ...
  {                                                                           // enter timed loop
    ...
    else if (PWM_leftMotor > 0){                          //Going backward
      motors.setSpeedB(abs(PWM_leftMotor));
      motors.backwardB();
    }
    else {                                               //Going forward
      motors.setSpeedB(abs(PWM_leftMotor));
    ...
    if (noCommLoops >= noCommLoopMax) {                   //Stopping if too much time without command
      ...
      motors.setSpeedA(abs(PWM_rightMotor));
      motors.forwardA();
    }
    else {                                                //Going backward
      motors.setSpeedA(abs(PWM_rightMotor));
